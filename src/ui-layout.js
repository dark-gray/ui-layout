...
/**
 * UI.Layout 1.4.2 (customized)
 * https://github.com/angular-ui/ui-layout
 */
angular.module('ui.layout', ['ngCookies'])
  .controller('uiLayoutCtrl', ['$scope', '$attrs', '$element', '$timeout', '$window', 'LayoutContainer', 'Layout', '$cookies',
    function uiLayoutCtrl($scope, $attrs, $element, $timeout, $window, LayoutContainer, Layout, $cookies) {
          ...
          if(splitbarIndex > -1) {
            ...
            if(!beforeContainer.collapsed && !afterContainer.collapsed) {
              ...
              debounceEvent = $timeout(function() {
                if (beforeContainer.cookieName) {
                  $cookies[beforeContainer.cookieName]= beforeContainer.size + "px";
                }
                if (afterContainer.cookieName) {
                  $cookies[afterContainer.cookieName]= afterContainer.size + "px";
                }
      ...
      ctrl.mouseMoveHandler = function(mouseEvent) {
        var mouseProperty = ctrl.sizeProperties.mouseProperty;
        var mousePos = mouseProperty in mouseEvent ? mouseEvent[mouseProperty]
          : mouseEvent.originalEvent && mouseProperty in mouseEvent.originalEvent ? mouseEvent.originalEvent[mouseProperty]
          : mouseEvent.targetTouches ? mouseEvent.targetTouches[0][mouseProperty]
          : mouseEvent.originalEvent && mouseEvent.originalEvent.targetTouches ? mouseEvent.originalEvent.targetTouches[0][mouseProperty]
          : null;
        ...
        if(ctrl.containers.length > 0 && $element.children().length > 0) {
          ...
          for(i=0; i < ctrl.containers.length; i++) {
            if(!LayoutContainer.isSplitbar(ctrl.containers[i])) {
              ...
              c = ctrl.containers[i];
              opts.sizes[i] = c.isCentral && !c.collapsed ? 'auto' : c.collapsed ? (optionValue(c.minSize) || '0px') : optionValue(c.uncollapsedSize) || 'auto';
      ...
      ctrl.toggleBefore = function(splitbar) {
        ...
        $scope.$apply(function() {
          if(c.collapsed) {
            ...
            c.size = c.minSize || 0;
            var sizeDiff = c.uncollapsedSize - c.size;

            if(nextSplitbar) {
              nextSplitbar[flowProperty] -= sizeDiff;
              nextSplitbar.collapsed = true;
            }
            if(nextContainer) {
              nextContainer[flowProperty] -= sizeDiff;
              nextContainer.uncollapsedSize = nextContainer.size += sizeDiff;  // uncollapsedSize is used in calculate()
            }
        ...
        if (c.cookieName) {
          $cookies[c.cookieName]= c.size + "px";
        }
        if (nextContainer.cookieName) {
          $cookies[nextContainer.cookieName]= nextContainer.size + "px";
        }
      ...
      ctrl.toggleAfter = function(splitbar) {
        ...
        $scope.$apply(function() {
          if(c.collapsed) {
            ...
            if(prevSplitbar) {
              prevSplitbar[flowProperty] += (sizeDiff + endDiff);
              prevSplitbar.collapsed = true;
            }
            if(prevContainer) {
              prevContainer.uncollapsedSize = prevContainer.size += (sizeDiff + endDiff);  // uncollapsedSize is used in calculate()
            }
        ...
        if (prevContainer.cookieName) {
          $cookies[prevContainer.cookieName]= prevContainer.size + "px";
        }
    ...
    return {
      ...
      link: function(scope, element, attrs, ctrl) {
        ...
        prevButton.on('mousedown', function(event) {
          ...
          var hidePreviousSplitbarButtons = function() {
            if(previousSplitbar !== null) {
          ...
          var showPreviousSplitbarIcons = function() {
            if(previousSplitbar !== null) {
              if (previousSplitbar.collapsed) {
        ...
        afterButton.on('mousedown', function(event) {
          event.stopPropagation();
          ...
          var hideNextSplitbarButtons = function() {
            if(nextSplitbar !== null) {
          ...
          var showNextSplitbarButtons = function() {
            if (nextSplitbar !== null) {
              if (nextSplitbar.collapsed) {
        ...
        element.on('mousedown touchstart', function(e) {
          ...
          htmlElement.on('mouseup touchend', function(event) {
            ...
            htmlElement.off('mouseup touchend');
          });
          return false;
        });
  ...
  .directive('uiLayoutContainer',
    ['LayoutContainer', '$compile', '$timeout', 'Layout', '$cookies',
      function(LayoutContainer, $compile, $timeout, Layout, $cookies) {
        return {
          restrict: 'AE',
          require: '^uiLayout',
          scope: {
          ...
          compile: function() {
            return {
              pre: function(scope, element, attrs, ctrl) {
                ...
                var cookieSize = $cookies[scope.cookieName] || scope.size;
                scope.container.size = cookieSize;
                scope.container.uncollapsedSize = cookieSize;
                scope.container.minSize = scope.minSize;
                scope.container.maxSize = scope.maxSize;
                scope.container.cookieName = scope.cookieName;
